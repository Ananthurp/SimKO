...
# --- TS2 Helper: Pure PyTorch Sparsemax ---
def sparsemax(logits, dim=-1):
    """
    Pure PyTorch implementation of Sparsemax to avoid external dependencies.
    Projects logits onto the probability simplex with a sparse support.
    Used for TS2 dynamic support set identification.
    """
    # 1. Translate logits for numerical stability (Sparsemax is shift-invariant)
    #    This ensures we don't hit NaNs with large logits
    ...
    # 4. Calculate threshold function: 1 + k * z_k > cumsum(z)
    cumsum_sorted = torch.cumsum(sorted_logits, dim=dim)
    is_gt = (1 + k_range * sorted_logits > cumsum_sorted).float()

    # 5. Determine support size k(z) = max k such that condition holds
    k_z = torch.sum(is_gt, dim=dim, keepdim=True)

    # 6. Calculate tau
    #    tau = (cumsum(z_k) - 1) / k
    #    Note: k_z indices are 1-based, subtract 1 for 0-based indexing
    tau_z = (torch.gather(cumsum_sorted, dim=dim, index=k_z.long() - 1) - 1) / k_z

    # 7. Compute probability
    p = torch.clamp(logits - tau_z, min=0)
    return p
# ------------------------------------------
...
def compute_policy_loss_simko(old_log_prob, old_log_probs_topk, log_prob, topk_log_probs, entropy, advantages, eos_mask,
                              cliprange, token_level_scores, max_token, mix_topk_coef=0.01, tau=1.0):
    """
    Updated SimKO Loss incorporating TS2 (Sparsemax) logic for positive updates.
    
    Logic:
    1. Positive Responses: Mix the sampled policy update with the average update of the *Sparsemax Support Set*.
       (Original SimKO used fixed Top-K average; TS2 uses dynamic Sparsemax support).
    2. Negative Responses: Apply SimKO's stronger penalty to the Top-1 token if incorrect.
    """
    correct_idx = token_level_scores.sum(-1) == 1
    # incorrect_idx = token_level_scores.sum(-1) == 0 # unused in this function currently

    # --- TS2 Logic: Compute Support Set Ratio ---
    # We use topk_log_probs to approximate the full distribution for sparsemax.
    # Assumes topk_log_probs contains enough candidates (e.g., K=16+) to capture the sparse support.
    
    log_prob_diff_topk = topk_log_probs - old_log_probs_topk
    ratios_topk = torch.exp(log_prob_diff_topk) # Ratio for each token in Top-K

    # Compute Sparsemax on the Top-K log_probs
    # Sparsemax(logits) is shift invariant, so Sparsemax(log_probs) is valid.
    sparse_probs = sparsemax(topk_log_probs, dim=-1)
    
    # Identify the "Plausible Set" (Support Set)
    support_mask = (sparse_probs > 0).float()
    support_size = support_mask.sum(dim=-1)
    
    # Compute the average ratio of the support set
    # Sum of ratios in support / Size of support
    # clamp support_size to avoid division by zero
    avg_support_ratio = (ratios_topk * support_mask).sum(dim=-1) / torch.clamp(support_size, min=1.0)
    # --------------------------------------------

    # Prepare masks for SimKO mixing logic
    B, T = log_prob.shape
    if correct_idx.dim() == 1 and correct_idx.size(0) == T:
        correct_mask = correct_idx.unsqueeze(0).expand(B, -1)   # (B,T)
    else:
    ...
    # Calculate Entropy Threshold (SimKO Logic)
    threshold = row_quantile_masked(entropy, eos_mask_bool, q=tau)
    threshold = threshold.view(-1, 1)

    # Determine "Forking" tokens (High Entropy)
    w = (entropy > threshold).float()
    ...
    negative_approx_kl = log_prob - old_log_prob
    ratio_taken = torch.exp(negative_approx_kl)
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, eos_mask)

    # --- Apply TS2 Mixing ---
    # If correct & high entropy: Ratio = (1-alpha)*Taken + alpha*Sparsemax_Support_Average
    ratio = torch.where(
        correct_mask,
        mix_main_pos * ratio_taken + mix_topk_pos * avg_support_ratio,
        ratio_taken
    )

    # --- Apply SimKO Negative Penalty ---
    if eos_mask.dtype != torch.bool:
        eos_mask_bool = (eos_mask > 0)
    else:
        eos_mask_bool = eos_mask
    ...
    # Penalty Condition: Top-1 Token chosen AND Advantage < 0 (Wrong) AND High Entropy
    neg_mask = (max_token > 0) & neg_mask & (entropy > threshold)
    
    scale = torch.ones_like(scores)
    # Apply 1.1x penalty scale (from original SimKO code)
    scale = scale.masked_fill(neg_mask, 1.1)
    advantages = advantages * scale
