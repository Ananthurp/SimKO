...
# 1. CUDA_VISIBLE_DEVICES=2,3 - Use only GPU 2 and GPU 3
# 2. trainer.n_gpus_per_node=2 - Changed from 8 to 2
# 3. rollout.tensor_model_parallel_size=2 - 2-GPU tensor parallelism for vLLM
# 4. data.train_batch_size=64 - Reduced from 1024 (extreme memory optimization)
# 5. data.max_response_length=1024 - Reduced from 3072 (extreme memory optimization)
# 6. actor.ppo_mini_batch_size=16 - Reduced from 256 (extreme memory optimization)
# 7. actor.ppo_max_token_len_per_gpu=2000 - Reduced from 12000 (extreme memory optimization)
# 8. rollout.log_prob_max_token_len_per_gpu=2000 - Reduced from 12000 (extreme memory optimization)
# 9. ref.log_prob_max_token_len_per_gpu=2000 - Reduced from 12000 (extreme memory optimization)
...
#
# Note: PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True is NOT compatible with vLLM 0.12.0's
# memory pool allocator and has been removed.
